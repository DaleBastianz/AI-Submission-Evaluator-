...
export default function SubmitAssignmentPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const storedUser = getStoredSessionUser() ?? session?.user;
      if (storedUser?.email) formData.append('email', storedUser.email);
      if (storedUser?.name) formData.append('name', storedUser.name);
      ...
      const response = await apiFetch('/api/submit-assignment', {
        method: 'POST',
        body: formData
      });
          ...
          <div className="mb-10 rounded-[2rem] border border-white/10 bg-slate-950/80 p-8 shadow-glow">
            <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-3 text-slate-400">Submitting as: <span className="font-medium text-white">{session?.user?.name || session?.user?.email}</span></p>
              </div>
              <div className="flex flex-wrap gap-3">
                <a href="/dashboard" className="rounded-full border border-white/10 px-5 py-3 text-sm text-slate-300 transition hover:border-cyan-400/60">Dashboard</a>
